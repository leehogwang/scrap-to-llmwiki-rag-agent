...
ClipWiki는 **점차 쌓아가며 강해지는 RAG**를 목표로 한 크롬 익스텐션 + 웹 대시보드입니다. 단순한 RAG처럼 매번 흩어진 스크랩 원문을 다시 읽는 대신, 웹에서 `Alt + Drag`로 모은 증거를 Notion에 저장하고, 이를 점차 위키 초안과 Graphify 지식 그래프로 구조화합니다. 즉 스크랩이 쌓이고 정리될수록 retrieval 계층 자체가 더 강해집니다.

## 개요

ClipWiki는 **기존 RAG의 한계를 보완하는 학습 흐름**을 목표로 합니다.

- 웹에서 공부하다가 필요한 부분을 빠르게 스크랩
- 원문 링크와 이미지를 포함한 상태로 Notion에 보관
- 저장된 스크랩을 검색
- 스크랩과 위키 초안을 함께 기반으로 질문
- 누적 스크랩을 구조화된 위키 초안으로 변환해 점점 강해지는 retrieval layer 만들기
- 결과 지식을 인터랙티브 그래프로 탐색

기존 RAG는 자료를 가져오는 데는 강하지만, 실제 사용에서는 몇 가지 한계가 있습니다.

- 질문할 때마다 raw text를 크게 다시 읽혀 토큰 비용이 커지기 쉽고
- 가져온 chunk들이 평평하게 나열되어 구조가 약하며
- 지식이 장기적으로 축적되어 재사용 가능한 층으로 남기 어렵고
- 서로 다른 문서 사이의 관련 아이디어를 명시적으로 연결하기 어렵고
- 답변은 만들 수 있어도 다음 질문을 더 잘하게 만드는 정리된 지식층은 잘 남기지 못합니다

ClipWiki는 이를 보완하기 위해:

- raw scrap을 그대로 증거로 보존하고
- 이를 위키 지식으로 재구성하며
- 새 스크랩이 오면 기존 위키를 확장하고
- Graphify로 숨은 연결성을 드러냅니다
...
- 점차 강해지는 RAG 구조
  - raw scrap은 근거로 남기고
  - wiki draft는 재사용 가능한 지식층이 되며
  - 비슷한 새 scrap은 기존 wiki를 보강하는 쪽으로 흡수
- YouTube 지원
...
- 단발성 retrieval이 아니라 **축적될수록 성능과 구조가 좋아지는 RAG**를 지향
